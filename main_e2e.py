...
from xiaowen_feeder_kitti_centertrack import Feeder
...
# from xiaowen_feeder_lyft import Feeder
...
work_dir = '../workdir/kitti_mots_centertrack'
...
def train_model(pra_model, pra_data_loader, pra_optimizer, pra_epoch_log):
    ...
    for iteration, (ori_data_ctt, ori_data_gt, A, _) in enumerate(pra_data_loader):
        # print(iteration, ori_data.shape, A.shape)
        # ori_data: (N, C, T, V)
        # C = 11: [frame_id, object_id, object_type, position_x, position_y, position_z, object_length, pbject_width, pbject_height, heading] + [mask]
        data, no_norm_loc_data, object_type = preprocess_data(
            ori_data_ctt, rescale_xy)
        data_gt, no_norm_loc_data_gt, object_type = preprocess_data(
            ori_data_gt, rescale_xy)
        # print(data[0,:,0,0])
        for now_history_frames in range(1, data.shape[-2]):
            # (N, C, T, V)=(N, 4, 6, 120) ---> N*120, 4, 6 ---> N*120, 2, 6 --> (N, 2, 6, 120)
            input_data = data[:, :, :now_history_frames, :]
            # gt_data = no_norm_loc_data_gt[:, :2,
            #                               now_history_frames:, :].clone()
            # # Starting from the 2nd frame in the future, GT object locations are substracted by its previous frame
            # gt_data[:, :, 1:] = gt_data[:, :, 1:] - gt_data[:, :, :-1]
            # # The 1st frame in the future is substracted by the current observation (CenterTrack's prediction)
            # gt_data[:, :, 0] = gt_data[:, :, 0] - \
            #     no_norm_loc_data[:, :2, now_history_frames-1].clone()

            output_loc_GT = data_gt[:, :2, now_history_frames:, :]
 ...
 #           output_mask = data[:, -1:, now_history_frames:, :]
            output_mask = data_gt[:, -1:, now_history_frames:,
            ...
            # (N, C, T, V)=(N, 2, 6, 120)
            predicted = pra_model(
                pra_x=input_data, pra_A=A, pra_pred_length=output_loc_GT.shape[-2], pra_teacher_forcing_ratio=0, pra_teacher_location=output_loc_GT)
            ...
            # (N, T), (N, T)
            overall_sum_time, overall_num, _ = compute_RMSE(
                predicted, output_loc_GT, output_mask, pra_error_order=1)
            # overall_loss
            total_loss = torch.sum(
                overall_sum_time) / torch.max(torch.sum(overall_num), torch.ones(1,).to(dev))  # (1,)

            now_lr = [param_group['lr']
                      for param_group in pra_optimizer.param_groups][0]
            my_print('|{}|{:>20}|\tIteration:{:>5}|\tLoss:{:.8f}|lr: {}|'.format(
                datetime.now(), pra_epoch_log, iteration, total_loss.data.item(), now_lr))
...
def val_model(pra_model, pra_data_loader):
    ...
    for iteration, (ori_data_ctt, ori_data_gt, A, _) in enumerate(pra_data_loader):
        ...
        data, no_norm_loc_data, _ = preprocess_data(
            ori_data_ctt, rescale_xy)
        data_gt, no_norm_loc_data_gt, _ = preprocess_data(
            ori_data_gt, rescale_xy)
        ...
        for now_history_frames in range(6, 7):
            ...
            ori_output_loc_GT = no_norm_loc_data_gt[:,
                                                    :2, now_history_frames:, :]
            ori_output_last_loc = no_norm_loc_data[:, :2,
                                                   now_history_frames-1:now_history_frames, :]

            # for category
            # (N, C, T, V)=(N, 1, 6, 120)
            cat_mask = ori_data_ctt[:, 2:3, now_history_frames:, :]
            ...
            overall_sum_time, overall_num, x2y2 = compute_RMSE(
                predicted, ori_output_loc_GT, output_mask)
            ...
            # car_mask = (cat_mask==0).float().to(dev)
            car_mask = output_mask * car_mask
            car_sum_time, car_num, car_x2y2 = compute_RMSE(
                predicted, ori_output_loc_GT, car_mask)
            ...
            # human_mask = (cat_mask==1).float().to(dev)
            human_mask = output_mask * human_mask
            human_sum_time, human_num, human_x2y2 = compute_RMSE(
                predicted, ori_output_loc_GT, human_mask)
            ...
            # bike_mask = (cat_mask==2).float().to(dev)
            bike_mask = output_mask * bike_mask
            bike_sum_time, bike_num, bike_x2y2 = compute_RMSE(
                predicted, ori_output_loc_GT, bike_mask)
    ...
    result_car = display_result(
        [np.array(all_car_sum_list), np.array(all_car_num_list)], pra_pref='car')
    result_human = display_result(
        [np.array(all_human_sum_list), np.array(all_human_num_list)], pra_pref='human')
    result_bike = display_result(
        [np.array(all_bike_sum_list), np.array(all_bike_num_list)], pra_pref='bike')

    # result = 0.20*result_car + 0.58*result_human + 0.22*result_bike
    # result = 0.5*result_car + 0.5*result_human
    result = (result_car + result_human + result_bike) / 3
    overall_log = '|{}|[{}] All_All: {}'.format(datetime.now(), 'WS', ' '.join(
        ['{:.3f}'.format(x) for x in list(result) + [np.sum(result)]]))
    my_print(overall_log)
...
def test_model(pra_model, pra_data_loader):
    ...
    with open(test_result_file, 'w') as writer:
        ...
        for iteration, (ori_data, A, mean_xy) in enumerate(pra_data_loader):
            ...
            ori_output_last_loc = no_norm_loc_data[:,
                                                   :2, history_frames-1:history_frames, :]

            A = A.float().to(dev)
            predicted = pra_model(pra_x=input_data, pra_A=A, pra_pred_length=future_frames,
...
def run_trainval(pra_model, pra_traindata_path, pra_valdata_path, pra_testdata_path=None):
    loader_train = data_loader(pra_traindata_path, pra_batch_size=batch_size_train,
                               pra_shuffle=True, pra_drop_last=True, train_val_test='train')
    loader_val = data_loader(pra_valdata_path, pra_batch_size=batch_size_val,
                             pra_shuffle=False, pra_drop_last=False, train_val_test='val')
    if pra_testdata_path is not None:
        loader_test = data_loader(pra_testdata_path, pra_batch_size=batch_size_train,
                                  pra_shuffle=True, pra_drop_last=True, train_val_test='all')
...
def run_test(pra_model, pra_data_path):
    loader_test = data_loader(pra_data_path, pra_batch_size=batch_size_test,
                              pra_shuffle=False, pra_drop_last=False, train_val_test='test')
    test_model(pra_model, loader_test)


if __name__ == '__main__':
    graph_args = {'max_hop': 2, 'num_node': 60}
    model = Model(in_channels=3, graph_args=graph_args,
    ...
    run_trainval(model, pra_traindata_path='../data/kitti_mots_centertrack/train_data.pkl',
                 pra_valdata_path='../data/kitti_mots_centertrack/val_data.pkl')
    ...
    # run_test(model, '/data/vci-dut/train_data.pkl')
