...
def preprocess_data(pra_data, pra_data_gt, pra_rescale_xy):
    # pra_data: (N, C, T, V)
    # C = 11: [frame_id, object_id, object_type, position_x, position_y, position_z, object_length, pbject_width, pbject_height, heading] + [mask]	
    # feature_id = [3, 4, 9, 10]
    feature_id = [3, 4, 5]
    ori_data = pra_data[:,feature_id].detach()
    ori_data_gt = pra_data_gt[:,feature_id].detach()
    ...
    ori_data_gt = ori_data_gt.float().to(dev)
    object_type = object_type.to(dev) #type
    data[:,:2] = data[:,:2] / pra_rescale_xy

    return data, ori_data, ori_data_gt, object_type
...
def train_model(pra_model, pra_data_loader, pra_optimizer, pra_epoch_log):
    ...
    for iteration, (ori_data_ctt, ori_data_gt, A, _) in enumerate(pra_data_loader):
        ...
        # ori_data: (N, C, T, V)
        # C = 11: [frame_id, object_id, object_type, position_x, position_y, position_z, object_length, pbject_width, pbject_height, heading] + [mask]
        data, no_norm_loc_data, no_norm_loc_data_gt, object_type = preprocess_data(ori_data_ctt, ori_data_gt, rescale_xy)
        # print(data[0,:,0,0])
...
def val_model(pra_model, pra_data_loader):
    ...
    for iteration, (ori_data_ctt, ori_data_gt, A, _) in enumerate(pra_data_loader):
        ...
        data, no_norm_loc_data, no_norm_loc_data_gt, _ = preprocess_data(ori_data_ctt, ori_data_gt, rescale_xy)
        ...
        for now_history_frames in range(6, 7):
            ...
            ori_output_loc_GT = no_norm_loc_data_gt[:,:2,now_history_frames:,:]
            ori_output_last_loc = no_norm_loc_data[:,:2,now_history_frames-1:now_history_frames,:]

            # for category
            cat_mask = ori_data_ctt[:,2:3, now_history_frames:, :] # (N, C, T, V)=(N, 1, 6, 120)
...
if __name__ == '__main__':
    ...
    model = Model(in_channels=3, graph_args=graph_args, edge_importance_weighting=True)
    model.to(dev)
    ...
    run_trainval(model, pra_traindata_path='../data/kitti_mots_centertrack/train_data.pkl', pra_valdata_path='../data/kitti_mots_centertrack/val_data.pkl')
