...
data_root = 'vci-dataset-dut/data/trajectories_filtered'
...
max_num_object = 180 # maximum number of observed objects is ？？
...
total_feature_dimension = 5 + 1 # we add mark "1" to the end of each row to indicate that this row exists

# after zero centralize data max(x)=127.1, max(y)=106.1, thus choose 130

def get_frame_instance_dict(pra_file_path_1, pra_file_path_2):
	'''
	Read raw data from files and return a dictionary: 
		...
		{frame_id: 
			{object_id: 
				# features
				[frame_id, object_id, 'label', position_x, position_y]
			}
		}

		'label' => 1: pedestrian, 2: vehicle
	'''
	now_dict = {}
	max_id = -1
	with open(pra_file_path_1, 'r') as csv_file:
		reader = csv.reader(csv_file)
		header = next(reader) #['id', 'frame', 'label', 'x_est', 'y_est', 'vx_est', 'vy_est']
		for row in reader:
			if( int(row[1])%12 == 1):
				row[0],row[1] = int(int(row[1])/12)+1, int(row[0])
				row[3],row[4] = float(row[3]), float(row[4])
				row = row[:5]#['frame', 'id', 'label', 'x_est', 'y_est']
				row[2] = 1 # ped:1, veh:2
	...
	with open(pra_file_path_2, 'r') as csv_file:
		reader = csv.reader(csv_file)
		header = next(reader) #['id', 'frame', 'label', 'x_est', 'y_est', 'vx_est', 'vy_est']
		for row in reader:
			if(int(row[1])%12 == 1):
...
def generate_train_data(pra_file_path_1, pra_file_path_2):
	'''
	...
	'''
	now_dict = get_frame_instance_dict(pra_file_path_1, pra_file_path_2)
	frame_id_set = sorted(set(now_dict.keys()))
...
def generate_data(pra_file_path_list, pra_is_train=True):
	...
	all_adjacency = []
	all_mean_xy = []
	for file_path_1, file_path_2 in zip(pra_file_path_list[::2],pra_file_path_list[1::2]): #改成两个为一组 
		# print(file_path_1,file_path_2)
		# process the sorted trajectories list two elementes per loop
		if pra_is_train:
			now_data, now_adjacency, now_mean_xy = generate_train_data(file_path_1,file_path_2)
		# else:
	...
	# if the file size is less than 4gb, use pickle.dump
	with open(save_path, 'wb') as writer:
		pickle.dump([all_data, all_adjacency, all_mean_xy], writer)
	# otherwise use byte write. since pickle.dump cannot handle larger than 4gb file
	# with open(save_path, 'wb') as writer:
	# 	bytes_out = pickle.dumps([all_data, all_adjacency, all_mean_xy])
	# 	max_bytes = 2**31 - 1
	# 	for idx in range(0, len(bytes_out), max_bytes):
	# 		writer.write(bytes_out[idx:idx+max_bytes])


if __name__ == '__main__':
	train_file_path_list = sorted(glob.glob(os.path.join(data_root, '*.csv'))) 
