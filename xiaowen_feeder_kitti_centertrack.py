...
class Feeder(torch.utils.data.Dataset):
	...
	def __init__(self, data_path, graph_args={}, train_val_test='train'):
		...
		total_num = len(self.all_feature_ctt)
	...
	def load_data(self):
		with open(self.data_path, 'rb') as reader:
			...
			[self.all_feature_ctt, self.all_feature_gt, self.all_adjacency, self.all_mean_xy]= pickle.load(reader)
			

	def __len__(self):
		return len(self.all_feature_ctt)

	def __getitem__(self, idx):
		# C = 11: [frame_id, object_id, object_type, position_x, position_y, position_z, object_length, pbject_width, pbject_height, heading] + [mask]
		now_feature_ctt = self.all_feature_ctt[idx].copy() # (C, T, V) = (11, 12, 120)
		now_feature_gt = self.all_feature_gt[idx].copy()
		...
		if self.train_val_test.lower() == 'train' and np.random.random()>0.5:
			...
			xy = now_feature_ctt[3:5, :, :]
			xy_gt = now_feature_gt[3:5, :, :]
			# xy = now_feature_ctt[0:2, :, :]
			...
			out_xy_gt = np.einsum('ab,btv->atv', angle_mat, xy_gt)
			now_mean_xy = np.matmul(angle_mat, now_mean_xy)
			xy[:,:,:num_xy] = out_xy[:,:,:num_xy]
			xy_gt[:,:,:num_xy] = out_xy_gt[:,:,:num_xy]

			now_feature_ctt[3:5, :, :] = xy
			now_feature_gt[3:5, :, :] = xy_gt
			# now_feature_ctt[0:2, :, :] = xy

		now_adjacency = self.graph.get_adjacency(self.all_adjacency[idx])
		now_A = self.graph.normalize_adjacency(now_adjacency)
		
		return now_feature_ctt, now_feature_gt, now_A, now_mean_xy
