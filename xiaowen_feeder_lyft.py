...
class Feeder(torch.utils.data.Dataset):
	...
	def __init__(self, data_path, graph_args={}, train_val_test='train'):
		...
		train_id_list = [i for i in range(total_num) if self.all_feature[i, 5, 0, 0] < 126]
		val_id_list = [i for i in range(total_num) if 126 <= self.all_feature[i, 5, 0, 0] < 144]
		test_id_list = [i for i in range(total_num) if 144 <= self.all_feature[i, 5, 0, 0] < 180]
		...
		if train_val_test.lower() == 'train':
			...
			self.all_feature = self.all_feature[test_id_list]
			self.all_adjacency = self.all_adjacency[test_id_list]
			self.all_mean_xy = self.all_mean_xy[test_id_list]
